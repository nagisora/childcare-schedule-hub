import {
	...
	normalizeFacilityNameForSearch,
	uniqueStrings,
...
export function generateSearchQueries(
	...
	const facilityVariants = uniqueStrings(
		buildFacilityNameVariantsForSearch(facilityName),
	);
	...
	if (genericFacilityName) {
		...
		if (wardName) {
			queries.push(
				`site:instagram.com ${facilityTerm} 名古屋 "${wardName}" 子育て instagram`,
			);
			queries.push(
				`site:instagram.com ${facilityTerm} 名古屋 "${wardName}" instagram`,
			);
			queries.push(
				`site:instagram.com ${facilityTerm} "${wardName}" 子育て instagram`,
			);
		}
...
export function scoreCandidate(
	...
	]).map((variant) => normalizeTextForMatch(variant));
	const genericFacilityName = isGenericFacilityName(facilityName);

	const hasFullMatch = facilityVariants.some(
		(variant) => variant.length >= 2 && titleAndSnippet.includes(variant),
	);
	if (hasFullMatch) {
		score += genericFacilityName ? 2 : 4;
		reasons.push(
			genericFacilityName ? "施設名一致（短い名称のため低め）" : "施設名一致",
		);
	} else {
		...
		const hasPartialMatch = facilityNameParts.some((part) =>
			titleAndSnippet.includes(part),
		);
		if (hasPartialMatch) {
	...
	const hasNagoyaContext = nagoyaKeywords.some((keyword) =>
	...
	const hasOtherArea = otherAreaKeywords.some((keyword) =>
		titleAndSnippet.includes(keyword),
	);
