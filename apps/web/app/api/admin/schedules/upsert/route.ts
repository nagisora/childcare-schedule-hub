...
export async function POST(request: NextRequest) {
	...
	const facilityId =
		typeof body.facility_id === "string" ? body.facility_id.trim() : "";
	...
	const urlRaw =
		typeof body.instagram_post_url === "string"
			? body.instagram_post_url.trim()
			: "";
	const instagramPostUrl = urlRaw
		? (normalizeInstagramPostUrl(urlRaw)?.normalized ?? null)
		: null;
	if (!instagramPostUrl) {
		return jsonError(
			400,
