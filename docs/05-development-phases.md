...
**注**: 現在は名古屋市の一部施設（65件）のみを対象としています。全施設の基本情報とInstagramアカウントURLのカバーは [フェーズ9: 施設情報の全面カバー](#フェーズ9-施設情報の全面カバー) で実施します。
...
## フェーズ9: 施設情報の全面カバー
- 目的: 一部の施設のみの状態から、全施設の基本情報とInstagramアカウントURLを対象としたデータ投入・更新フローを確立する
- 主に触るドキュメント: `02` / `03` / `04`
- 完了条件:
  - 全施設（名古屋市の全応援拠点・支援拠点）の基本情報が `facilities` テーブルに登録されている
  - 各施設のInstagramアカウントURLが可能な限り特定され、`facilities.instagram_url` に登録されている
  - 施設情報のデータ更新フロー（定期的な更新手順）が `04 開発ガイド` に Runbook としてまとまっている
...
フェーズ5で確立した手動フローを活用し、**全施設の基本情報とInstagramアカウントURLのカバー**を実現します。
...
- 完了条件:
  ...
  - スケジュールURLのデータ更新フロー（月次更新の手順）が `04 開発ガイド` に Runbook としてまとまっている
  - スケジュールURLのデータ品質チェック（有効性・更新漏れの確認）が実施されている

**注**: このフェーズでは、手動でのデータ投入を基本とし、可能な限り全施設のスケジュールURLをカバーすることを目指します。自動化の高度な実装（例: Instagram投稿の自動解析によるURL抽出）は、このフェーズ以降の発展的タスクとして検討します。

フェーズ6で確立した手動フローを活用し、**全施設のスケジュールURLのカバー**を実現します。
...
- **フェーズ9: 施設情報の全面カバー** - 全施設の基本情報とInstagramアカウントURLのデータ投入・更新フローの確立
- **フェーズ10: スケジュールURLの全面カバー** - 全施設の月間スケジュールURLのデータ投入・更新フローの確立
- **フェーズ11: デザイン / UX 仕上げ & リリース準備** - 本番さながらのクオリティに高める
