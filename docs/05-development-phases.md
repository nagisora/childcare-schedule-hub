...
## フェーズ9: InstagramアカウントURLの全面カバー
- 目的: 一部の施設のみの状態から、全施設のInstagramアカウントURLを対象としたデータ投入・更新フローを確立する
- 主に触るドキュメント: `docs/instagram-integration/` / `02` / `03`
- 完了条件:
  - 各施設のInstagramアカウントURLが可能な限り特定され、`facilities.instagram_url` に登録されている
  - InstagramアカウントURLの検索・登録フローが `docs/instagram-integration/` に Runbook としてまとまっている
...
- **フェーズ9: InstagramアカウントURLの全面カバー** - 全施設のInstagramアカウントURLのデータ投入・更新フローの確立
