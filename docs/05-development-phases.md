...
- フェーズ9: 仮MVP環境のクオリティアップ
- フェーズ10: 施設情報・スケジュールURLの全面カバー
...
**注**: 現在は名古屋市の一部施設（65件）のみを対象としています。全施設のカバーは [フェーズ10: 施設情報・スケジュールURLの全面カバー](#フェーズ10-施設情報スケジュールurlの全面カバー) で実施します。
...
**注**: 現在は一部の施設・スケジュールURLのみを手動で登録している状態です。全施設のスケジュールURLカバーは [フェーズ10: 施設情報・スケジュールURLの全面カバー](#フェーズ10-施設情報スケジュールurlの全面カバー) で実施します。
...
**注**: このフェーズは「仮MVP環境」として、一部の施設・スケジュールURLのみを手動で登録した状態でデプロイしています。本番相当のクオリティ向上は [フェーズ9: 仮MVP環境のクオリティアップ](#フェーズ9-仮mvp環境のクオリティアップ) で実施します。
...
**注**: フェーズ7では仮MVP環境として Vercel を利用していますが、本フェーズで本番デプロイ先を再検討します。Cloudflare Pages など、よりコスト効率の良い選択肢も検討対象となります。
...
このフェーズで「本番さながらのデザイン」を狙うため、フェーズ4では **骨格レベル**、フェーズ9では **エラーハンドリング・安定性**、フェーズ11で **ビジュアル / UX 最適化レベル** と段階を分けます。
...
- **フェーズ8: デプロイ先の検討（コスト最適化）** - 本番デプロイ先の選定（Vercel / Cloudflare Pages など）
- **フェーズ9: 仮MVP環境のクオリティアップ** - エラーハンドリング・アクセシビリティ・安定性の向上
- **フェーズ10: 施設情報・スケジュールURLの全面カバー** - 全施設のデータ投入・更新フローの確立
