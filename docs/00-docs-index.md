...
| **Supabase DB セットアップ（フェーズ3）を実行したい** | **`06` 1.x, 2.1, 4.1〜4.4, 3.1〜3.5** / `05` フェーズ3のチェックリスト<br>※ Cursor を使う場合は、06 の MCP セクション（4節）を参照しつつ AI に実行させることを推奨 |
| Supabase MCP の導入手順を知りたい | `06` 4.1〜4.4 節 |
