...
- AIと協力して開発する際の「壁打ち→計画→チェックリスト→実行内容の明記→実行」の流れを1ファイルで完結させる
...
- 対応フェーズ / セッション種別 / 実行方式（AI自律/ペア/手動） / 影響範囲 / 日付 / 想定所要時間
...
- **作業タスク & 実行内容**:
  - 各タスクを「1回の実行で完結する粒度」まで分解（AIに任せる場合は「1指示で完結」粒度）
  - 目安: **1タスク = 1実行単位 + 確認まで15〜30分**
  - 各タスクに「完了条件」と「AIが実行する内容（手順/プロンプト/操作メモ）」を記述
...
2. 前提・合意事項を記録
   ↓
3. 実装チェックリストを作成
   - タスクを1実行単位に分解（AIに任せる場合は1指示で完結できる粒度）
   - 各タスクの「AIが実行する内容（手順/プロンプト/操作メモ）」を明記
   ↓
4. チェックリストに沿って実装
   - 記載した実行内容をそのままAIへの指示として使ってもよい（必要なら調整）
   ↓
5. 検証・テストを実行
   ↓
6. 結果とふりかえりを記録
