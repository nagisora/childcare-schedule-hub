...
- **基本**: 過去セッションの「次回に持ち越すタスク」で挙がっていた項目が、後日の別セッションで完了した場合は、**元のセッション側も更新してクローズ**する。
  - **やること**:
    - 元のセッションファイルで対象タスクを `- [x]` にする
    - **完了日**と、完了したセッションへの**参照リンク**（dev-session）を付ける
      - 例: `- [x] ○○○ - 2025-12-14（[dev-session](./YYYYMMDD-..-....md)）`
    - （任意）追跡性を上げたい場合は **コミットハッシュ**も併記する
  - **狙い**: 「どのセッションで完了したか」が過去ログから即追える状態を作り、持ち越しが永遠に残り続けるのを防ぐ
