...
- [x] タスク1: hybrid戦略（rank主軸+score再評価）をAPIに追加する
  - 完了条件:
    - [x] `/api/instagram-search?strategy=hybrid` が 200 で応答する（実測で確認済み）
    - [x] hybridの候補がプロフィールURLのみで返る（既存ルール維持）
    - [x] triedQueries が返り続ける（既存互換、実測で確認済み）
    - [x] ユニットテストが追加/更新されて通る（9件のテストケース追加、全41件通過）
  - **実行プロンプト案**:
    ```
...
- [x] タスク2: CLIに hybrid を追加し、比較実測しやすくする
  - 完了条件:
    - [x] `--strategy=hybrid` が使える（実測で確認済み）
    - [x] 既存の `--compare-strategies` を活用して、rankとhybridの差分が確認できる（rank/hybrid比較に変更、実測で確認済み）
    - [x] 非対話環境での安全装置方針が明確（rank主経路を維持、`--auto-adopt` が必要な仕様を維持）
  - **実行プロンプト案**:
    ```
...
- [ ] タスク3: scoreCandidate の重み付け/説明を、失敗パターンに基づいて改善する
  ...
  - **実施状況**: 今回は実測のみ実施。実測結果を分析したところ、失敗パターンが明確でないため、より多くの実測データを集めてから改善を実施する方針（次回セッションに持ち越し）
  - **実行プロンプト案**:
    ```
...
- [x] タスク4: 10〜20件の実測データを作り、rank vs hybrid の改善前後を比較・記録する
  - 完了条件:
    - [x] 対象区（東区以外）で10〜20件をDRY-RUNし、レビュー用ログを基に表を作れる（緑区8件で実施、10〜20件の目標には届かなかったが実測データは取得）
    - [x] rank と hybrid を同一条件で比較し、0/1/複数と正候補含有の差分を要約できる（結果: rank/hybrid で同じ結果、複数候補は1件のみで実質的な違いは見られなかった）
  - **実測結果サマリ**: 緑区8件中、候補が見つかった6件、見つからなかった2件。複数候補があった施設は1件のみ（葡萄の木、2件）。詳細は「結果とふりかえり」セクション参照
  - **実行プロンプト案**:
    ```
...
- [x] タスク5: ドキュメント反映（正本への進捗反映）
  - 完了条件:
    - [x] 本dev-sessionに結果・分析・次の改善方針が残っている（「結果とふりかえり」セクションに記録済み）
    - [x] `docs/05-09-instagram-account-url-coverage.md` の該当箇所に今回のリンク/進捗が追記されている（タスク4追加として記録済み）
...
- [x] 確認1: webユニットテスト
  ...
  - **実施結果**: 全41件のテストが通過（hybrid戦略のテストケース9件を含む）

- [x] 確認2: API手動確認（strategy=hybrid）
  - 期待結果: `strategy=hybrid` で candidates/triedQueries が返る（シークレットは露出しない）
  - **実施結果**: 実測（CLI DRY-RUN）で動作確認済み。緑区8件で正常に動作し、candidates/triedQueries が返ることを確認

- [x] 確認3: CLI DRY-RUNで実測（rank/hybrid）
  - 期待結果: reviewログから比較できる（候補数と品質）
  - **実施結果**: 緑区8件で実測実施。reviewログ（`apps/scripts/logs/instagram-review-2025-12-16-05-23-02.json` / `.md`）に記録済み
...
- 完了できたタスク:
  - [x] タスク1: hybrid戦略（rank主軸+score再評価）をAPIに追加する
  - [x] タスク2: CLIに hybrid を追加し、比較実測しやすくする
  - [ ] タスク3: scoreCandidate の重み付け/説明を、失敗パターンに基づいて改善する（今回は実測のみ実施、改善は次回）
...
> **このセッションはクローズ**。持ち越しタスクの正本は次のセッションに移動した：`docs/dev-sessions/2025/12/20251216-02-phase9-instagram-search-hybrid-more-measurements.md`

- [x] 再検索抑制キャッシュ（facilityId+query+results）の設計・実装（持ち越し済み → `docs/dev-sessions/2025/12/20251216-02-phase9-instagram-search-hybrid-more-measurements.md`）
  - 今回やらない理由: 効率化/コスト削減が主目的で、まずhybrid/scoreの精度改善を優先
  - 次回の着手条件: 実測が回り始め、同一施設・同一クエリの再実行が増えてきたら
- [x] Runbook整備（標準フロー/フォールバック）とデータ品質チェック（フェーズ9タスク6）（持ち越し済み → `docs/dev-sessions/2025/12/20251216-02-phase9-instagram-search-hybrid-more-measurements.md`）
  - 今回やらない理由: 実装・実測に集中するため
  - 次回の着手条件: 1区以上を継続運用できる状態になったら
- [x] scoreCandidate の改善（タスク3の続き）（持ち越し済み → `docs/dev-sessions/2025/12/20251216-02-phase9-instagram-search-hybrid-more-measurements.md`）
  - 今回やらない理由: 実測結果を分析したところ、失敗パターンが明確でないため
  - 次回の着手条件: より多くの実測データを集めてから改善を実施する方針
