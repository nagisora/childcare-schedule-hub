...
- [ ] タスク1: 東区の半自動登録を更新モードで1回実施し、「処理済み」にする
...
- [ ] タスク2: データ品質チェック（SQL）を1回実行し、結果を証跡として残す
...
- [ ] 確認2: CLIのDRY-RUN比較（`--compare-strategies`）
      - 期待結果: score/rank の差分が分かり、`rank` の候補が妥当と判断できる
- [ ] 確認3: CLIの更新モード（`--apply --yes`）
...
- スタート: HH:MM
- メモ:
  - （ここに実行コマンド、結果ファイル名、件数、判断理由を短く記録）
