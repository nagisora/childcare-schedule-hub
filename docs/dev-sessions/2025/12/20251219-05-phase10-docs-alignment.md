...
- [ ] タスク8: フェーズ10の定義を詳細設計ができるレベルまで詳細に記載する
  ...
  - **AIが実行する内容（手順/プロンプト/操作メモ）**:
    ```
    - 参照ファイル:
      - docs/05-00-development-phases.md（フェーズ10節）
      - docs/02-design.md（schedules テーブル定義）
      - docs/03-api.md（ScheduleSummary）
      - docs/04-development.md（既存のRunbook）
      - docs/05-09-instagram-account-url-coverage.md（フェーズ9の参考例）
    - やりたいこと:
      - 対象範囲の定義を具体化（例: 対象月は何月から、対象施設の条件）
      - 処理済みの定義を明確化（登録済み / 未特定確定 / 対象外の各状態と判断基準）
      - 未特定理由の分類を具体化（見つからない / 判断不能 / 対象外など、各理由の定義）
      - データ登録パターンを明確化（どのパターンをどのカラムに登録するか、判断基準）
      - 検証方法・品質チェックの手順を具体的に記載（SQLクエリ例、確認手順など）
    - 制約・注意点:
      - 05-10詳細ファイルはまだ作らない（タスク9で作成する）
      - 実装の詳細ではなく、設計レベルの詳細を記載する
    ```
...
- 変更したドキュメント:
  ...
  - `docs/05-00-development-phases.md`: フェーズ11・12・13の完了条件
- 重要な判断:
