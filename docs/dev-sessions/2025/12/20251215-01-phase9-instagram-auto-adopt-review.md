...
- [ ] キャッシュ（facilityId+query+results）での再検索抑制
  - 注: これは精度向上のための機能ではなく、**効率化・コスト削減**のための機能
  - 同じ施設ID・同じクエリで複数回検索する場合、過去の検索結果をキャッシュして再検索を避けることで、Google CSE APIのクエリ数（1日100クエリ制限）を削減し、実行時間を短縮する
  - 例: `--compare-strategies` で同じ施設をscore/rankで検索する場合、キャッシュがあれば2回目のAPI呼び出しを回避できる
...
以下の指示を新しいAIエージェントセッションで実行してください。コンテキストをリフレッシュしてから進めることを推奨します。

```
フェーズ9のInstagram URL登録CLIの検索精度を向上させるため、実測データの拡大とクエリ改善を実施してください。

## 参照ドキュメント

- `docs/05-09-instagram-account-url-coverage.md`（フェーズ9正本、タスク5）
- `docs/dev-sessions/2025/12/20251215-01-phase9-instagram-auto-adopt-review.md`（前回セッション、自動採用機能実装完了）
- `docs/04-development.md`（9.5.3: CLI手順）
- `apps/scripts/instagram-semi-auto-registration.ts`（CLI実装）
- `apps/web/lib/instagram-search.ts`（検索クエリ生成ロジック）

## 作業目標

1. **検索精度の実測を10〜20件に拡大**し、成功/失敗パターンを記録する
2. **rank戦略のクエリ改善**（`名古屋市` の扱い等）を実装し、実測結果と比較して効果を検証する
3. 両方を並行して進め、実測しながら改善を試し、効果を同時に検証する

## 実施内容

### 1. 実測データの拡大（10〜20件）

- 対象区を選定（東区以外の区で、未登録施設が10〜20件程度ある区）
- `--ward=<区名> --strategy=rank --auto-adopt` でDRY-RUN実行
- 各施設について以下を記録:
  - 施設名
  - 候補数（0件 / 1件 / 2件以上）
  - 自動採用可否（候補1件のみなら自動採用、複数候補は未特定として記録）
  - 未特定の場合の理由コード（`auto_adopt_blocked_multiple_candidates` 等）
  - 候補の品質（正しい候補が含まれているか、誤検出があるか）
- `子育て拠点` キーワード追加の効果も測定する

### 2. rank戦略のクエリ改善

- `apps/web/lib/instagram-search.ts` の `generateSearchQueries()` を確認
- 現在のクエリ生成ロジックを理解し、改善案を検討
- 特に以下の点を検討:
  - `名古屋市` の扱い（区名と組み合わせる場合の優先度など）
  - 短い施設名の場合のクエリパターン
  - 複数候補が返りやすいケースの対策
- 改善案を実装し、実測データと比較して効果を検証する

### 3. 結果の記録と分析

- 実測結果を `docs/dev-sessions/2025/12/YYYYMMDD-XX-phase9-instagram-search-precision-improvement.md` に記録
- 成功パターンと失敗パターンを分類・分析
- クエリ改善の効果を定量的に評価
- 今後の改善方針を提案
...
- シークレット（APIキー/トークン）は絶対に表示・ログに出力しない
- DRY-RUNモードで実行し、実際のDB更新は慎重に（必要に応じて `--apply --yes` を検討）
- 実測結果は構造化して記録し、後から分析できるようにする
- クエリ改善は段階的に実施し、各変更の効果を検証する

## 完了条件

- [ ] 10〜20件の施設について実測データを記録している
- [ ] 成功/失敗パターンが分類・分析されている
- [ ] rank戦略のクエリ改善を実装している
- [ ] 改善前後の効果を比較・検証している
- [ ] dev-sessionに結果を記録している
- [ ] `docs/05-09-instagram-account-url-coverage.md` のタスク5セクションに進捗を反映している

## 補足情報

### 現在の実装状況

- `--auto-adopt` フラグで非対話環境での自動採用が可能
- `子育て拠点` / `子育て` キーワードがrank戦略に追加済み
- Markdown形式のレビューサマリファイルが出力される
- 東区3件の実測結果: 2件が自動採用可能（候補1件）、1件が複数候補で未特定
...
# DRY-RUN実行（実測用）
cd apps/scripts
pnpm tsx instagram-semi-auto-registration.ts --ward=<区名> --strategy=rank --auto-adopt

# 結果ファイルの確認
ls -t logs/instagram-review-*.md | head -1 | xargs cat
...
- dev-sessionファイル（実測結果・分析・改善内容）
- クエリ改善の実装（`apps/web/lib/instagram-search.ts`）
- ドキュメント更新（進捗チェックリストの更新）
```
