...
> 最新の持ち越し正本: `docs/dev-sessions/2025/12/20251214-02-phase9-instagram-apply-quality-check.md`

- [x] 検索精度の実測を 10〜20件に拡大し、成功/失敗パターンを記録（短い施設名ケースを含める）（持ち越し済み → [dev-session](./20251214-02-phase9-instagram-apply-quality-check.md)）
- [x] タスク5の「DB更新の安全装置（バックアップ/ロールバック/差分確認）」を強化して本運用へ（持ち越し済み → [dev-session](./20251214-02-phase9-instagram-apply-quality-check.md)）
- [x] キャッシュ（facilityId+query+results）での再検索抑制（持ち越し済み → [dev-session](./20251214-02-phase9-instagram-apply-quality-check.md)）
- [x] Runbook整備とデータ品質チェック（フェーズ9タスク6）（持ち越し済み → [dev-session](./20251214-02-phase9-instagram-apply-quality-check.md)）
- [x] rank戦略のクエリ改善（`名古屋市` の扱い等）とログ/比較の磨き込み（持ち越し済み → [dev-session](./20251214-02-phase9-instagram-apply-quality-check.md)）
- [x] 対象施設を「処理済み」にする（`instagram_url` 埋め/未特定（理由付き）一覧化、必要なら `--apply` でDB反映）（漏れていたため追加: 2025-12-14）（持ち越し済み → [dev-session](./20251214-02-phase9-instagram-apply-quality-check.md)）
- [x] データ品質チェック結果（重複URL/非instagramドメイン/投稿URL混入など）を dev-sessions に証跡として残す（SQL結果・件数）（漏れていたため追加: 2025-12-14）（持ち越し済み → [dev-session](./20251214-02-phase9-instagram-apply-quality-check.md)）
