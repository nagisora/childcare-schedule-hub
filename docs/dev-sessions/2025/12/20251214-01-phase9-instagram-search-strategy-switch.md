...
- [ ] 対象施設を「処理済み」にする（`instagram_url` 埋め/未特定（理由付き）一覧化、必要なら `--apply` でDB反映）（漏れていたため追加: 2025-12-14）
- [ ] データ品質チェック結果（重複URL/非instagramドメイン/投稿URL混入など）を dev-sessions に証跡として残す（SQL結果・件数）（漏れていたため追加: 2025-12-14）
