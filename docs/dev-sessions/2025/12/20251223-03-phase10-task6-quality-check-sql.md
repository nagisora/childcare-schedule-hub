...
- [x] タスク1: （前提）対象月のデータが不足している場合、最小スコープで投入（DRY-RUN→APPLY）してバックアップを作る
  - 完了条件: `apps/scripts/logs/` に JSON/Markdown と `schedules-backup-*.json` が生成される
  - 実施状況: 
    - DRY-RUNは実行済み（JSON/Markdownは生成）
    - APPLYは未実行（複数候補のため手動レビューが必要なため、バックアップファイルは未生成）
    - **補足**: CLIは正常に動作し、候補は取得できているが、3件すべて複数候補のため自動判定不可（手動レビューが必要）
...
- 完了できたタスク:
  - [x] タスク1: CLI DRY-RUNを実行し、ログファイル（JSON/Markdown）を生成（候補は取得できているが、複数候補のため自動判定不可）
...
- 未完了タスク / 想定外だったこと:
  - [ ] タスク1のAPPLY: データ投入（CLIのDRY-RUNで候補は取得できたが、すべて複数候補のため自動判定不可。手動レビューが必要なため、APPLYは未実行）
- 学び・次回改善したいこと:
...
## Instagramスケジュール投稿の取得状況（2025-12-01）

### 全体サマリ

- **対象施設数**: 54件（`facilities.instagram_url IS NOT NULL`）
- **登録済み**: 3件（既にDBに `instagram_post_url` が設定されている）
- **未処理**: 51件（まだ `schedules` 行が無い）

### 登録済み施設（3件）

1. **こころと**（昭和区）
   - `instagram_post_url`: `https://www.instagram.com/p/DRD9er5krYe/`

2. **つながるひろば恵方の家**（昭和区）
   - `instagram_post_url`: `https://www.instagram.com/p/DRLmvAzCeGw/`

3. **なっちゃん**（西区）
   - `instagram_post_url`: `https://www.instagram.com/p/DRi8crtj3hY/`

### CLI実行結果（2025-12-23 02:16:18実行、対象: 3施設）

CLIは正常に動作し、**候補は取得できています**が、すべて複数候補のため自動判定不可でした。

#### 1. おやこっこなか（中区）
- **ステータス**: `not_found`（理由コード: `S10_NOT_FOUND_MULTIPLE_CANDIDATES`）
- **候補数**: 26件
- **候補例**:
  - `https://www.instagram.com/p/CXNgCZiJ2io/`
  - `https://www.instagram.com/p/DSb4DlHCal9/`
  - `https://www.instagram.com/p/DBSg-6svO55/`
  - ...（他23件）
...
- **候補数**: 10件
- **候補例**:
  - `https://www.instagram.com/p/DDDyAekTShu/`
  - `https://www.instagram.com/p/DINRCxMztCw/`
  - `https://www.instagram.com/p/DF9PNVqTtpC/`
  - ...（他7件）
...
- **候補数**: 7件
- **候補例**:
  - `https://www.instagram.com/p/DReASPKEVnK/`
  - `https://www.instagram.com/p/DMwlGWFP1nP/`
  - `https://www.instagram.com/p/DKJXTQ0P-Q0/`
  - ...（他4件）

### 結論

- **CLIは正常に動作**: API呼び出し成功、候補URLは取得できている
- **自動判定は不可**: 3件すべて複数候補のため、手動レビューが必要
- **既存登録**: 3件の施設は既にDBに登録済み（以前のセッションで登録されたものと推測）
- **残り51件**: 未処理施設が51件あり、今後のデータ投入が必要

**次のステップ**: 複数候補が検出された施設については、手動レビューで適切な候補を選択し、`--apply --yes` でDBに反映する必要があります。

### 検索精度向上の改善（2025-12-23実施）

既存の3件の登録済み投稿を参考に、検索キーワードを改善しました。

**追加した月ヒント**:
- 「MM月号」（例: "12月号"）- よく使われるパターンとして優先度を上げました
- 「MM月カレンダー」（例: "12月カレンダー"）
- 「MM月おたより」（例: "12月おたより"）
- 「カレンダー」（単独）
- 「おたより」（単独）
...
**検索クエリの改善**:
- 検索クエリに「カレンダー OR おたより OR スケジュール」を追加
- 月ヒントの優先順位を改善（「月号」を優先的に使用）
- 月ヒントの最大パターン数を4から8に拡張（精度向上のため）

**実装ファイル**: `apps/web/lib/instagram-schedule-search.ts`
- `buildMonthHints` 関数を更新
- `generateScheduleSearchQueries` 関数を更新

**テスト結果**: 既存のユニットテスト（11テスト）はすべて通過

**次の確認**: 改善後の検索クエリでCLIを再実行し、複数候補が減るか、単一候補に収束するかを確認する必要があります。

### 検索精度向上の試行錯誤結果（2025-12-23実施）

検索キーワードを改善し、複数回の試行錯誤を実施しました。

**改善内容**:
1. **「月号」を最優先に使用**: 検索クエリで「12月号」を必須キーワードとして最優先に配置
2. **具体的なキーワードを追加**: 「カレンダー」「おたより」を個別に使用するクエリを追加
3. **候補の並び替え改善**: 月ヒントがマッチする候補を優先的に並べ替え（月号マッチ > 月ヒントマッチ > /p/優先）
4. **判定ロジック改善**: 月ヒントがマッチする候補のみを自動採用対象に変更
5. **フォールバッククエリ追加**: usernameありの場合もフォールバッククエリを追加

**試行錯誤の結果**:

| 施設名 | 初期候補数 | 最終候補数 | 改善状況 |
|---|---|---|---|
| おやこっこなか | 26件 | 3件 | 大幅改善（88%削減） |
| おひさまのおうち | 10件 | 2件 | 大幅改善（80%削減） |
| くれよんすたじお | 7件 | 0件 | 悪化（フォールバッククエリ追加済み） |

**最終実行結果（2025-12-23 02:46:01）**:
- **おやこっこなか**: 3件（月ヒントマッチ: 3件、全体: 3件）
  - 候補: `DO-3AM7CWBv`, `DNmiXGYyZd5`, `DKa-93zP8Ba`
- **おひさまのおうち**: 2件（月ヒントマッチ: 2件、全体: 2件）
  - 候補: `DRQ35-XEp7o`, `DNjyqn9yaWv`
- **くれよんすたじお**: 0件（CSE検索で候補が0件）
  - Instagram username: `crayonland2004`
  - フォールバッククエリを追加したが、まだ0件

**結論**:
- **大幅な精度向上**: 「おやこっこなか」と「おひさまのおうち」は候補数を大幅に削減できました
- **自動判定は不可**: まだ複数候補のため、手動レビューが必要です
- **「くれよんすたじお」の課題**: 0件になってしまったため、さらなる検索クエリの調整が必要です

**次のステップ**:
1. 「くれよんすたじお」の検索クエリをさらに調整（施設名のバリエーションや、より柔軟なキーワードを試す）
2. 複数候補が残る施設については、手動レビューで適切な候補を選択
3. 改善した検索クエリで全54施設を対象に実行し、自動判定できる割合を確認
