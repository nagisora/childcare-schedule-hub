...
## 7. デプロイと運用
1. **Git-driven運用**: `main` ブランチへのマージで自動デプロイが実行される。PR ごとにプレビュー環境（Vercel Preview）が自動生成される。
2. **本番デプロイ前の確認**: プレビュー URL で動作確認し、非機能要件を満たすかチェックする。
3. **環境変数管理**: ホスティング基盤の Environment（現状: Vercel の Preview / Production）に分けて登録する。
4. **トラブル時の確認**: `supabase logs` やホスティング基盤のログ（現状: Vercel ログ）を確認し、影響度を即時に共有する。
...
- **ロールバック（Git-driven運用）**: 
  - **基本手順**: 重大障害時は `git revert` で問題のあるコミットを取り消し、`main` にマージして再デプロイを実行する。これにより、直前の正常な状態に戻る。
  - **DB restore**: 原則として行わない。データベースのロールバックが必要な緊急時のみ、別途手順・判断基準に従って `supabase db restore` を実行する（通常はコードのロールバックで対応可能）。
  - **補助手段**: Vercel ダッシュボードから直前成功ビルドへのロールバックも可能だが、Git-driven運用を優先する。
- 注: 本番デプロイ先は [フェーズ12: デプロイ先の検討（コスト最適化）](./05-00-development-phases.md#フェーズ12-デプロイ先の検討コスト最適化) で再検討する。ホスティングを変更した場合は、本節の手順を移植・更新すること。
