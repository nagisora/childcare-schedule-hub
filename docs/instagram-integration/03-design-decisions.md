...
2025-01-22（調査・検討中 - 追加調査が必要な項目あり）
...
- 詳細ページにInstagramリンクがあるかは未確認（要調査）
...
- **詳細ページの確認後に判断**
  - サンプル数件の詳細ページを確認し、Instagramリンクの有無を確認
  - リンクがある場合は、スクレイピングスクリプトの拡張を検討（将来の拡張）
...
**PoC実装**:
- **キャプションからURLを抽出するスクリプト**:
  - Instagram投稿のキャプションからURLを抽出（正規表現等）
  - Googleドライブ、自治体サイト等のURLパターンを認識
  - 抽出したURLを `schedules.instagram_post_url` に自動登録
- **詳細ページ経由のスクレイピング**:
  - 詳細ページにInstagramリンクがある場合、スクレイピングスクリプトを拡張
  - `facilities.instagram_url` を自動取得

**自動化の範囲**:
- **自動化可能**: 
  - InstagramアカウントURLの取得（詳細ページにリンクがある場合）
  - キャプション内URLの抽出（投稿形式が統一されている場合）
- **人手が必要**:
