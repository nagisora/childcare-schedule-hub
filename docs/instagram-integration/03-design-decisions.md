...
- 詳細ページについては、**2025-01-22時点のサンプル3件ではInstagramリンクが確認できなかった**が、**2025-12-16に「Instagram URLが明示されている詳細ページ」が存在することを確認**した（例: `https://www.kosodate.city.nagoya.jp/play/supportbases10.html`）。
...
- **一次ソース優先（更新）**:
  - まず `facilities.detail_page_url` の詳細ページからInstagram URLを抽出し、明示されているものはそれを正として登録する
    - **注意**: `detail_page_url` は `https://www.kosodate.city.nagoya.jp/play/...` が正。`/play/` 抜けは 404 になり得るため、巡回前に正規化（補完）する
  - 抽出できない施設のみ、検索API（Google CSE）で補完する（rank主経路 + レビュー運用）
- **補足（2025-01-22時点の判断）**:
  - 当時はサンプル3件で未検出だったため「詳細ページからの自動取得は困難」と判断していたが、現状はその断定を撤回する
