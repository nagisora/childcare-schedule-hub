...
### 気づき・課題
- **名古屋市サイトからの自動取得は「一部可能」**: 一覧ページにはInstagramリンクがないが、詳細ページにはInstagram URLが明示されているケースがある（例: `https://www.kosodate.city.nagoya.jp/play/supportbases10.html`）。一次ソース抽出を先に回すと、検索の無駄を減らせる
