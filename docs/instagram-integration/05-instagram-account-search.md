...
- **検索結果ページでのURLの見つけ方**:
  - 検索結果ページには、各検索結果が `<h3 class="LC20lb MBeuO DKV0Md">` という見出し要素（`<h3>`）で表示される。
  - この見出し要素には、**ページタイトルとリンクが含まれている**。
  - 例: `<h3 class="LC20lb MBeuO DKV0Md">名古屋市中川区子育て応援拠点ゆるまる ...</h3>` という見出しには、Instagramアカウントへのリンクが含まれている。
  - **具体的な手順**:
    1. ブラウザツールのスナップショット機能（`browser_snapshot`）を使用して検索結果ページの構造を取得する。
    2. スナップショット結果から、`role="heading"` かつ `level=3` の要素（`<h3>`要素）を探す。
    3. 各 `<h3>` 要素の親要素（通常は `role="link"` の要素）の `url` 属性を確認する。
    4. `url` 属性に `instagram.com/...` が含まれている要素を特定する。
    5. その `<h3>` 要素の `name` 属性（見出しテキスト）に、施設名・エリア情報・「子育て応援拠点」などのキーワードが含まれているかを確認する。
    6. 条件を満たす `<h3>` 要素が見つかったら、その親要素の `url` 属性から Instagram アカウントURL を取得する。
  - **注意**: スナップショット取得自体が目的ではない。スナップショットから `<h3>` 要素を探し、リンクを確認することが目的である。
- **公式と思われる候補の特定**:
