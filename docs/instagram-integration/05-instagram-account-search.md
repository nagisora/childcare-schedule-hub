...
> **補足（検索APIによる自動化の検討状況）**
>
> 本ドキュメントは、**Google Custom Search API を使った標準フロー**と、**フォールバックとしてのブラウザ手動検索フロー**の両方を記載している。
> - **標準フロー**: `apps/scripts/instagram-semi-auto-registration.ts` を使用した半自動登録（推奨）
> - **フォールバック**: ブラウザでGoogle検索を開く手順（標準フローが使えない場合）
...
### 4.0 標準フロー: Google Custom Search API を使った半自動登録（推奨）
...
3. 採用候補を確認してDB更新:
   ...
   ```
4. 複数候補や未特定の施設は、レビュー用サマリファイル（`apps/scripts/logs/instagram-review-*.md`）を確認して手動判断
...
### 4.5 公式と判断できる候補がない場合の「あきらめ条件」と記録方法
...
- **記録方法**:
  - `instagram_url` は更新せず `NULL` のままにする
  - 未特定確定の理由を dev-session や Runbook に記録する（例: 「施設名 + 区 + 子育て支援拠点で検索したが、Instagramアカウントが見つからず」）
  - 再調査が必要かどうかも明記する（例: 「再調査: 任意（公式サイトや詳細ページの確認を推奨）」）
  - 未特定確定施設一覧を `docs/05-09-instagram-account-url-coverage.md` の「8. 未特定確定施設一覧」セクションに記録する
...
## 9. 検索API版ワークフロー（フェーズ9以降、2025-12-13追加）

> **注意**: このセクションは、Google Custom Search API を使用した検索フローの案です。  
> 現行の正式手順は上記「4. Instagramアカウント検索 手順サマリ（AI向け）」を参照してください。  
> 検索API版が十分に安定した後、標準フローとして切り替えることを検討します。
...
1. **最優先**: `site:instagram.com "<施設名>" "<区名>" 子育て`
   - 例（東区「あおぞらわらばぁ～」の場合）: `site:instagram.com "あおぞらわらばぁ～" "東区" 子育て`
...
### 9.5 検索API版フローとブラウザ手動フローの使い分け

- **標準フロー**: Google Custom Search API を使用（本セクション 9.1〜9.4）
- **フォールバック**: ブラウザ手動検索フロー（上記「4. Instagramアカウント検索 手順サマリ」）を使用
  - 検索APIが利用できない場合（レート制限、API障害など）
  - 検索APIで候補が見つからないが、人間が目視で確認したい場合

---

**最終更新日**: 2025-12-13（検索API版ワークフローを追加: Google Custom Search API を使用した検索クエリ設計と判定ルール）
