...
### 2.2 初期準備（pnpm 主体 / mise 任意）
このプロジェクトでは、日常のコマンド実行を **pnpm** に統一します。  
Node.js / pnpm のバージョン固定を行いたい場合のみ、**mise を任意で利用**します。
...
pnpm install
   ```
   - mise を使う場合は `mise exec -- pnpm install` でも実行可能。
...
- 注: `apps/web/package.json` の name が `web` のため、`--filter web` でフィルタする。mise を使う場合は `mise exec -- pnpm --filter web dev` でも可。
...
| `pnpm --filter web test:coverage` | カバレッジ取得 |
| `pnpm --filter web e2e` | E2E テスト（Playwright） |
| `pnpm --filter web dev` | フロントエンドの開発サーバー |
| `pnpm --filter web build` | プロダクションビルド |
| `pnpm --filter web format:check` | Biome 形式チェック（段階導入中） |
| `pnpm --filter web format` | Biome 形式整形（段階導入中） |

注: mise を使う場合は各コマンドの先頭に `mise exec --` を付けて同様に実行可能。

### 5.2.1 Biome 導入方針（段階導入中）
- 現在は **format 先行導入** とし、lint は既存の ESLint（`next lint`）を継続する。
- 目的は「整形コマンドの統一」と「差分肥大化の回避」であり、ESLint 置換は次段階で判断する。
- 当面はリファクタ対象ファイルを中心に適用し、既存コード全体への一括整形は行わない。
...
- LLM 生成コードは ESLint と Biome format を通し、根拠（参照仕様）をコメントとして残す。

### 5.6 テスト戦略
- **単体テスト (Vitest)**: UI コンポーネントとユーティリティを対象に `pnpm --filter web test` を実行し、`__tests__` ディレクトリへ配置。
- **型チェック (TypeScript)**: `pnpm --filter web typecheck` を CI で実行し、ゼロエラーを維持。
- **統合テスト (Playwright)**: 拠点一覧表示・お気に入り操作など主要フローを `apps/web/tests/e2e` に実装。`pnpm --filter web e2e` で実行し、CI ではヘッドレスモードとトレース保存を有効化。
- **アクセシビリティ検証**: Storybook 上で `@axe-core/react` を利用し、週次で Lighthouse CI を実行。
- **カバレッジ目標**: 単体 70% / 重要ユースケース 100% を目標に `pnpm --filter web test --coverage` を実行。閾値を下回った場合はビルドを失敗させる。
...
#### Vercel ダッシュボードでのログ確認
1. **確認箇所**: Vercel ダッシュボード > プロジェクト > Deployments > 該当デプロイ > Logs
2. **確認対象**:
   - ビルドエラー（`npm run build` のエラー出力）
   - ランタイムエラー（Next.js のサーバーエラー・クライアントエラー）
   - API Route のエラーレスポンス（500エラー・タイムアウト）
3. **典型エラーの見方**:
   - 環境変数未設定: `Missing Supabase environment variables` などのエラーメッセージを確認
   - Supabase接続エラー: `Failed to fetch` や `401 Unauthorized` などのネットワークエラーを確認
   - ビルドエラー: TypeScript型エラー・ESLintエラー・依存関係のエラーを確認

#### Supabase ダッシュボードでのログ確認
1. **確認箇所**: Supabase ダッシュボード > Logs > API Logs / Postgres Logs
2. **確認対象**:
   - クエリエラー（SQL構文エラー・制約違反）
   - RLS ポリシー違反（`new row violates row-level security policy`）
   - レート制限エラー（`429 Too Many Requests`）
3. **典型エラーの見方**:
   - RLS ポリシー未設定: `new row violates row-level security policy` エラーが発生した場合、該当テーブルの RLS ポリシーを確認
   - クエリタイムアウト: 長時間実行されるクエリがある場合、インデックスの追加やクエリの最適化を検討
   - レート制限: Supabase の無料枠上限に達している場合、キャッシュ戦略の見直しや有料プランへの移行を検討

### 7.2 稼働監視（Uptimeチェック）の設定（フェーズ12）

#### 監視ツールの選定
- **候補ツール**: UptimeRobot（無料枠あり）/ Better Uptime / Vercel Analytics の Uptime 機能
- **注意**: 外部サービスのアカウント作成・課金が必要な場合は「手動作業」として別タスクに切り出す

#### 監視設定手順
1. **監視対象URL**: 本番URLのトップページ（例: `https://childcare-schedule-hub.vercel.app`）
2. **監視間隔**: 5分間隔（無料枠の制限に応じて調整）
3. **失敗時の通知先**: 
   - メールアドレス（必須）
   - Slack Webhook（任意、設定可能な場合）
4. **失敗時の確認手順**:
   1. Vercel ダッシュボードでデプロイログを確認（[7.1 ログ確認手順](#71-ログ確認手順フェーズ12) 参照）
   2. Supabase ダッシュボードでログを確認（[7.1 ログ確認手順](#71-ログ確認手順フェーズ12) 参照）
   3. ブラウザで直接アクセスしてエラー内容を確認
   4. 環境変数の設定を確認（Vercel ダッシュボード > Settings > Environment Variables）

#### 監視設定の記録
- 監視設定完了後、dev-sessions に以下を記録:
  - ツール名
  - 監視URL
  - 監視間隔
  - 通知先（メールアドレス・Slack Webhook URL は機密情報のため、設定済みである旨のみ記録）

### 7.3 CodeRabbit の導入（フェーズ12）

#### 導入理由
- **フェーズ11で導入する場合**: リリース準備段階でコードレビューを強化し、リリース前の品質を担保する
- **フェーズ12で導入する場合**: 本番運用開始後の継続的なコード品質チェックに活用する

#### 導入手順
1. [CodeRabbit](https://www.coderabbit.ai/ja) にアクセスし、アカウントを作成（GitHub連携）
2. リポジトリを選択し、CodeRabbit を有効化
3. 設定項目を確認:
   - PR レビューの自動実行
   - レビュー対象ファイル（`.ts` / `.tsx` / `.md` など）
   - レビューコメントの通知設定
4. **注意**: アカウント作成・課金が必要な場合は「手動作業」として別タスクに切り出す

#### 導入完了の記録
- 導入完了後、dev-sessions に以下を記録:
  - 導入日
  - 設定内容（レビュー対象・通知設定など、機密情報を除く）

### 7.4 公開前の漏洩チェック（フェーズ12）

#### チェックリスト
公開前に以下を確認し、問題があれば修正してからリリースする:

- [ ] `.env` / `.env.local` が Git にコミットされていない（`.gitignore` で除外されている）
  - 確認方法: `git ls-files | grep -E '\.env'` で `.env` ファイルが検出されないことを確認
- [ ] `env.local.example` に実際のシークレットが含まれていない（ダミー値のみ）
  - 確認方法: `apps/web/env.local.example` を開き、実際のAPIキー・トークンが含まれていないことを確認
- [ ] コード内にハードコードされた認証情報（APIキー・トークン・パスワード）がない
  - 確認方法: `grep -r "eyJ" apps/` などで JWT トークンがハードコードされていないことを確認
- [ ] ログ出力に認証情報が含まれていない（`console.log` で環境変数を直接出力していない）
  - 確認方法: `grep -r "console.log.*process.env" apps/` で環境変数の直接出力がないことを確認
- [ ] 個人情報（実名・メールアドレス・電話番号）がコード・ドキュメント・コミットメッセージに含まれていない
  - 確認方法: コード・ドキュメント・コミット履歴を確認し、個人情報が含まれていないことを確認

#### チェック実施の記録
- チェック実施後、dev-sessions に以下を記録:
  - 実施日
  - 確認項目（チェックリストの各項目）
  - 問題の有無（問題があった場合は修正内容も記録）

## 8. トラブルシューティング
| 問題 | 対処 |
| --- | --- |
| Instagram 埋め込みが表示されない | 投稿 URL と oEmbed レスポンスを確認し、[03 API 仕様](./03-api.md) の手順通りに再取得。連続失敗は Edge Function の通知・`instagram_errors` ログを確認。埋め込み失敗時は自動的にフォールバック表示（投稿URLへの直接リンク）が表示される。 |
| Supabase REST が 401/403 を返す | RLS ポリシーと `apikey` ヘッダー、Service Role 利用箇所を確認。匿名アクセスが許可されているか `auth.uid()` をログ出力して検証。エラー時はページ内にエラーメッセージと再試行ボタンが表示される。 |
...
| Playwright テストが失敗する | `pnpm --filter web e2e --headed --debug` でデバッグし、`page.waitForLoadState('networkidle')` を活用。CI ではトレース保存を有効化。 |
...
| トップページで「エラーが発生しました」が表示される | Supabase への接続エラーまたはデータ取得エラーの可能性。ブラウザの開発者ツール（F12）でコンソールエラーを確認。環境変数（`NEXT_PUBLIC_SUPABASE_URL`、`NEXT_PUBLIC_SUPABASE_ANON_KEY`）が正しく設定されているか確認。再試行ボタンをクリックしてページを再読み込み。 |
| 拠点詳細ページで「拠点が見つかりませんでした」が表示される | 無効なIDまたはデータが存在しない可能性。URLのIDが正しいか確認。Supabase Studio で `facilities` テーブルに該当データが存在するか確認。 |
| お気に入りセクションでスケジュールが表示されない | スケジュールデータが未登録の可能性。Supabase Studio で `schedules` テーブルに該当施設のデータが存在するか確認。ローディング表示が長時間続く場合は、ブラウザの開発者ツールでネットワークエラーを確認。 |
...
フェーズ8で実装したエラーハンドリング・ローディング・空状態のUIを活用したトラブルシューティング手順。

#### 9.1.1 ユーザーから見える症状と確認ポイント

| 症状 | 確認ポイント | 対処方法 |
| --- | --- | --- |
| トップページに「エラーが発生しました」が表示される | ブラウザの開発者ツール（F12）でコンソールエラーを確認 | 環境変数（`NEXT_PUBLIC_SUPABASE_URL`、`NEXT_PUBLIC_SUPABASE_ANON_KEY`）が正しく設定されているか確認。Supabase プロジェクトのダッシュボードで接続状態を確認。 |
| 拠点一覧が空（「拠点データが登録されていません」） | Supabase Studio で `facilities` テーブルを確認 | データが存在する場合は、ISR キャッシュの問題の可能性。`/api/revalidate` を実行してキャッシュを無効化。 |
| お気に入りセクションで「スケジュールを読み込み中...」が長時間表示される | ブラウザの開発者ツールのネットワークタブでリクエストを確認 | `schedules` テーブルへのアクセスエラーの可能性。Supabase Studio で RLS ポリシーを確認。 |
| Instagram投稿の埋め込みが「埋め込みに失敗しました」と表示される | Instagram投稿URLが有効か確認 | 投稿が削除されていないか、アカウントが非公開になっていないか確認。投稿URLを直接ブラウザで開いて確認。 |

#### 9.1.2 開発者が確認すべきログ・ダッシュボード

- **Supabase Studio**: Table Editor で `facilities` / `schedules` テーブルのデータを確認
- **Supabase Studio**: Logs タブでエラーログを確認（API リクエストエラー、RLS ポリシー違反など）
- **Vercel ダッシュボード**: Deploy ログでビルドエラーを確認
- **ブラウザの開発者ツール**: コンソールエラー、ネットワークタブでリクエスト/レスポンスを確認

#### 9.1.3 簡易チェックリスト

代表フローの動作確認時に確認すべき項目:

- [ ] トップページが正常に表示される（エラーメッセージが表示されない）
- [ ] 拠点一覧が表示される（空状態メッセージが表示されない）
- [ ] お気に入り追加ボタン（「+」）が動作する
- [ ] お気に入りセクションに追加した施設が表示される
- [ ] スケジュールが表示される（または「スケジュールが登録されていません」メッセージが表示される）
- [ ] Instagram投稿の埋め込みが表示される（またはフォールバック表示が表示される）
- [ ] 拠点詳細ページが正常に表示される（無効なIDの場合は「拠点が見つかりませんでした」が表示される）
...
- `apps/web/.env.local` に以下が設定されていること:
  ...
  - `GOOGLE_CSE_CX`
  - `ADMIN_API_TOKEN`
- Next.js 開発サーバーが起動していること（`pnpm --filter web dev`）
...
4. 以下の項目を入力:
   ...
   - `image_url`: スケジュール画像のURL（必須）
     - **暫定運用**: まずはダミーURLでも可（例: `https://example.com/schedule-2025-01.jpg`）
     - **注意**: MVP UIでは `image_url` を表示に使用しない（Instagram埋め込み失敗時は投稿URLリンクを表示）。DBスキーマ上の必須カラムとして保持し、将来的なフォールバック表示や運用上の整合性確保のために設定する
...
3. **フォールバック表示（MVP）**:
   - Instagram投稿URLを直接リンクとして表示（「Instagramで開く」リンク）
   - **注意**: MVP UIでは `schedules.image_url` を表示に使用しない（DB必須だが、UI表示は未使用）
