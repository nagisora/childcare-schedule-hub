...
- 画像/静的ホスティング: Supabase Storage + ホスティング基盤（現状: Vercel、本番デプロイ先はフェーズ11で再検討）
- インフラ運用: ホスティング基盤（現状: Vercel、CI/CD）、GitHub Actions（補助タスク）、Supabase バックエンド
  - 注: 本番デプロイ先の選定は [フェーズ12: デプロイ先の検討（コスト最適化）](./05-00-development-phases.md#フェーズ12-デプロイ先の検討コスト最適化) で実施。Cloudflare Pages などに変更する場合は、本節をアップデートすること。
