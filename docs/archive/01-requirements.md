...
2. 「よく使う拠点」は最大5件を表示対象とし、このエリアでのみInstagram埋め込みを表示する。埋め込み失敗時は投稿URLへの直接リンクを表示する（MVPでは `image_url` をUI表示に使用しない）。
...
| 目標値 | 検証手段 |
| --- | --- |
| CI/CD による自動デプロイと段階的ロールアウト（現状: Vercel、本番デプロイ先はフェーズ11で再検討） | GitHub Actions ワークフロー（[04 開発ガイド](./04-development.md)） |
...
- ホスティング基盤は仮MVP環境として Vercel を利用しているが、本番デプロイ先は [フェーズ11: デプロイ先の検討（コスト最適化）](./05-00-development-phases.md#フェーズ11-デザイン--ux-仕上げ--リリース準備) で再検討する（候補: Cloudflare Pages など）。
