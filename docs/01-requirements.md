...
### 6.1 MVP フェーズ
- 拠点一覧と詳細情報を提供し、手動登録したスケジュール画像または Instagram 埋め込みを表示する。
- ブラウザクッキーでお気に入り拠点を保存し、トップページ上部に並び替えて表示する。
- 非ログイン前提での利用に最適化し、モバイル優先のレスポンシブ UI を提供する。
...
### 7.1 MVP 機能要件
1. トップページに拠点一覧を表示し、名称・エリア・連絡先・最新スケジュールを提示する（[02 設計資料](./02-design.md) の UI 設計と整合）。
2. 各拠点カードで最新のスケジュール画像または Instagram 埋め込みを表示する。管理画面が未実装の間は手動入力（Supabase 直接投入）で対応する。
3. お気に入り機能はクッキーで拠点 ID を保存し、並び替えと固定表示を行う（仕様は [03 API 仕様](./03-api.md) のクッキー要件と一致させる）。
4. レスポンシブレイアウトを適用し、スマートフォン・タブレット・PC で可読性と操作性を確保する。
5. エンプティステート・エラーメッセージ・アクセシビリティ要件を満たした UI を提供する（アクセシビリティ要件は [02 設計資料](./02-design.md) を参照）。
...
## 9. ユーザーストーリー（MVP）
1. 保護者として、よく利用する拠点の最新スケジュールをトップページで素早く確認したい。
2. 保護者として、お気に入り登録した拠点を一覧上部に固定し、すぐにアクセスしたい。
3. 支援団体として、新しい拠点の所在地や連絡先を検索し、問い合わせたい。
