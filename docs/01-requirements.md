...
- ブラウザクッキーでお気に入り拠点（最大5件）を保存し、トップページ上部に固定・並び替えて表示する（DB 同期はポストMVP）。
...
3. お気に入り機能はクッキーで拠点 ID と `sortOrder` を保存し、並び替え・固定表示を行う。最大5件、JSON 形式（[03 API 仕様](./03-api.md) のクッキー要件と一致）。
...
- データ要件: お気に入りはクッキー（最大5件, `SameSite=Lax; Path=/; Max-Age=15552000`。本番は `Secure` 推奨）で保持され、デバイス間で同期しない旨を UI 上に明示する。
