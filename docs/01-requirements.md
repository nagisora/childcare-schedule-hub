...
## 6. 非機能要件
### 6.1 パフォーマンス
- ISR（Incremental Static Regeneration）を活用し、一覧ページの初回表示を 3 秒以内にする。
- サーバーコンポーネント／エッジキャッシュを活用し、Instagram 埋め込み以外のデータ取得は Supabase キャッシュ層で高速化する。
