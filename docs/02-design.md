...
1. **トップページ**
   ...
   - よく使う拠点（最大5件）: クッキー保存された拠点をカード/リストで表示。将来的にこのエリアでのみスケジュール画像/埋め込みを表示。
   - 拠点一覧（テキスト表）: 拠点名 / エリア / 住所 / 電話 / 「+」ボタン（お気に入りに追加）。モバイルでは縦積み表示に崩す。スケジュール画像/埋め込みは一覧では表示しない。
2. **拠点詳細ページ（ポストMVP）**
   - 基本情報、月ごとのスケジュール画像/Instagram 埋め込みをまとめて表示。トップの「よく使う拠点」からの導線を設ける。
   - URL 構成: `apps/web/app/facilities/[id]/page.tsx` を想定。
   - アクセシビリティ: `iframe` には `title` と説明文、`loading=\"lazy\"`、必要に応じて `sandbox` を付与。
