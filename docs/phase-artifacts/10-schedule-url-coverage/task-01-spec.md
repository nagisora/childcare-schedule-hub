...
> 補足: このフローは **MVPの自動処理（CLI）** を想定しており、Instagramページの自動巡回（固定投稿/ハイライト判定のための巡回）は避ける前提です。  
> **手動レビュー**を行う場合の推奨順は **ピン留め（固定投稿）→ハイライト→Google CSE** です（ただし固定/ハイライトであっても「対象月の月間スケジュール」の根拠が取れない場合は採用しない）。
...
4. CSE検索の実施
   ...
   └─ 参照不能/ハイライトのみ等が根拠として取れる → 対象外（`S10_OUT_OF_SCOPE_*`。例: `S10_OUT_OF_SCOPE_STORY_OR_HIGHLIGHT_ONLY`）
   ↓
5. 出力（JSON/Markdown）
   - 登録済み: schedules に反映
